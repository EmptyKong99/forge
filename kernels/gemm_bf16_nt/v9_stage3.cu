...
// v9 (PTX route): v8 + 3-stage cp.async pipeline (deeper prefetch).
// v8 double-buffers (2 stages); big shapes (8192^2) still sat ~0.91x = tensor-core
// feed stalls. A 3rd in-flight K-tile hides more global latency. ldmatrix / mma /
// epilogue are byte-for-byte v8 — ONLY the buffering changes. 3 stages need 60KB
// shared (> the 48KB static cap), so As/Bs move to dynamic shared.
...
gemm_v9(const __nv_bfloat16* __restrict__ A,
  ...
  const int nk = K / BK;
  // prime STAGES-1 tiles in flight
  #pragma unroll
  ...
  for (int t = 0; t < nk; ++t) {
    __pipeline_wait_prior(STAGES - 2);   // tile t ready; STAGES-2 still loading
    __syncthreads();
    int cur = t % STAGES;
    ...
    int nxt = t + STAGES - 1;           // prefetch the tile STAGES-1 ahead
...
extern "C" cudaError_t openkernels_launch_gemm_bf16_nt(
  ...
  size_t smem = (size_t)STAGES * (BM + BN) * BKP * sizeof(__nv_bfloat16);  // 60KB
  static bool set = false;
  if (!set) {
    cudaFuncSetAttribute(gemm_v9, cudaFuncAttributeMaxDynamicSharedMemorySize, (int)smem);
    set = true;
  }
  dim3 block(THREADS);
  dim3 grid(N / BN, M / BM);
  gemm_v9<<<grid, block, smem, stream>>>(
