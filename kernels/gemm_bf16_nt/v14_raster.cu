...
// v14 (PTX route): v13 + L2-aware threadblock rasterization.
// v13 is compute-good but the big shapes (8192^2, 16384) lag square_4096 — they're
// DRAM/L2-bound on A/B reads. Default row-major block scheduling sends concurrent
// CTAs marching across N (same A rows, different B cols), so B streams from DRAM.
// Rasterize instead: launch a 1D grid and remap linear bid -> (tile_m, tile_n) in
// GROUP-tall column strips, so consecutive (concurrent) CTAs share the SAME tile_n
// = the SAME B columns -> B hits in L2. Classic CUTLASS threadblock swizzle.
// Everything else (swizzled shared, stmatrix epilogue) is byte-for-byte v13.
...
#define BKP 32
#define THREADS 256
#define GROUP 8         // rasterization strip height, in tile-rows
...
gemm_v14(const __nv_bfloat16* __restrict__ A,
         const __nv_bfloat16* __restrict__ B,
         __nv_bfloat16* __restrict__ C,
         int M, int N, int K,
         long asm_, long bsn, long csm, long csn,
         float alpha, float beta) {
  // --- L2 rasterization: linear bid -> (tile_m, tile_n) in GROUP-tall strips ---
  const int tiles_m = M / BM, tiles_n = N / BN;
  const int bid = blockIdx.x;
  const int blocks_per_group = GROUP * tiles_n;
  const int group_id = bid / blocks_per_group;
  const int first_m = group_id * GROUP;
  const int gsize_m = min(GROUP, tiles_m - first_m);     // ragged last group
  const int idx = bid % blocks_per_group;
  const int tile_m = first_m + (idx % gsize_m);
  const int tile_n = idx / gsize_m;
...
extern "C" cudaError_t openkernels_launch_gemm_bf16_nt(
  ...
  dim3 block(THREADS);
  dim3 grid((M / BM) * (N / BN));        // 1D grid, remapped in-kernel
  gemm_v14<<<grid, block, 0, stream>>>(
