...
gemm_v7(const __nv_bfloat16* __restrict__ A,
  ...
  for (int t = 0; t < nk; ++t) {
    ...
    for (int ks = 0; ks < BK; ks += 16) {
      ...
      // A fragments: 4 M-tiles of 16 rows. ldmatrix.x4 16x16 each.
      #pragma unroll
      for (int mt = 0; mt < 4; ++mt) {
        int rowbase = warp_row * 64 + mt * 16 + (lane & 15);
        int kcol = ks + ((lane >> 4) & 1) * 8;
        ldm_x4(a[mt][0], a[mt][1], a[mt][2], a[mt][3], smem_addr(&As[cur][rowbase][kcol]));
      }
      // B fragments: 4 N-tiles of 8 cols. ldmatrix.x2.trans from Bs[n][k].
      #pragma unroll
      for (int nt = 0; nt < 4; ++nt) {
        int nrow = warp_col * 32 + nt * 8 + (lane & 7);
        int kcol = ks + ((lane >> 3) & 1) * 8;
        ldm_x2(b[nt][0], b[nt][1], smem_addr(&Bs[cur][nrow][kcol]));
      }
      #pragma unroll
      for (int mt = 0; mt < 4; ++mt)
        #pragma unroll
