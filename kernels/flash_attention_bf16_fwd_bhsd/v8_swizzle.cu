...
// v8 swizzle — v7 + XOR swizzle, NO padding (the gemm v13 lever). v7 used DP=136
// padding to dodge ldmatrix bank conflicts; v8 drops the pad (DPS=128) and permutes
// the 8-elem chunk by row bits instead (occupancy-neutral conflict cure). Applied
// identically at every store (incl. cp.async dst) and ldmatrix load, on Qs/Ks/Vs
// (128-wide, swz128) and Ps (16-wide, swz16). D=128, S%16==0.
...
__device__ __forceinline__ int swz128(int row, int c) { return (((c >> 3) ^ (row & 7)) << 3) | (c & 7); }
__device__ __forceinline__ int swz16(int row, int c)  { return (((c >> 3) ^ (row & 1)) << 3) | (c & 7); }
...
__device__ __forceinline__ void ldm2t(uint32_t& a, uint32_t& b, uint32_t s) {
...
fa_v8(const __nv_bfloat16* __restrict__ Q, const __nv_bfloat16* __restrict__ K,
  ...
  __shared__ __nv_bfloat16 Qs[QPB][DPS];
  __shared__ __nv_bfloat16 Ks[2][BN][DPS];
  __shared__ __nv_bfloat16 Vs[2][BN][DPS];
  __shared__ __nv_bfloat16 Ps[QPB][PW];

  const long qb = b * qsb + h * qsh, kb = b * ksb + h * ksh, vb = b * vsb + h * vsh;
  for (int i = tid; i < QPB * DM; i += WARPS * 32) {
    int r = i / DM, c = i % DM;
    Qs[r][swz128(r, c)] = (qblock + r < S_q) ? Q[qb + (long)(qblock + r) * qss + c * qsd] : __float2bfloat16(0.f);
  }
  ...
  auto load = [&](int buf, int kt) {
    for (int i = tid; i < BN * DM; i += WARPS * 32) {
      ...
      if ((c & 7) == 0) {
        if (kp < S_kv) {
          __pipeline_memcpy_async(&Ks[buf][r][swz128(r, c)], &K[kb + (long)kp * kss + c * ksd], 16);
          __pipeline_memcpy_async(&Vs[buf][r][swz128(r, c)], &V[vb + (long)kp * vss + c * vsd], 16);
        } else {
          *reinterpret_cast<int4*>(&Ks[buf][r][swz128(r, c)]) = make_int4(0, 0, 0, 0);
          *reinterpret_cast<int4*>(&Vs[buf][r][swz128(r, c)]) = make_int4(0, 0, 0, 0);
        }
      }
    }
    __pipeline_commit();
  ...
  for (int t = 0; t < ntile; ++t) {
    ...
    for (int kk = 0; kk < 8; ++kk) {
      ...
      uint32_t a0, a1, a2, a3; ldm4(a0, a1, a2, a3, sm(&Qs[qrow][swz128(qrow, dcol)]));
      int kcol = kk * 16 + ((lane >> 3) & 1) * 8;
      int kr0 = (lane & 7), kr1 = 8 + (lane & 7);
      uint32_t b0, b1;
      ldm2(b0, b1, sm(&Ks[cur][kr0][swz128(kr0, kcol)]));   mma(s0, s1, s2, s3, a0, a1, a2, a3, b0, b1, s0, s1, s2, s3);
      ldm2(b0, b1, sm(&Ks[cur][kr1][swz128(kr1, kcol)]));   mma(u0, u1, u2, u3, a0, a1, a2, a3, b0, b1, u0, u1, u2, u3);
    }
    ...
    Ps[pr0][swz16(pr0, keyidx[0])] = __float2bfloat16(p[0]); Ps[pr0][swz16(pr0, keyidx[1])] = __float2bfloat16(p[1]);
    Ps[pr0][swz16(pr0, keyidx[2])] = __float2bfloat16(p[2]); Ps[pr0][swz16(pr0, keyidx[3])] = __float2bfloat16(p[3]);
    Ps[pr1][swz16(pr1, keyidx[0])] = __float2bfloat16(p[4]); Ps[pr1][swz16(pr1, keyidx[1])] = __float2bfloat16(p[5]);
    Ps[pr1][swz16(pr1, keyidx[2])] = __float2bfloat16(p[6]); Ps[pr1][swz16(pr1, keyidx[3])] = __float2bfloat16(p[7]);
    __syncwarp();

    int prow = warp * 16 + (lane & 15);
    uint32_t pa0, pa1, pa2, pa3; ldm4(pa0, pa1, pa2, pa3, sm(&Ps[prow][swz16(prow, ((lane >> 4) & 1) * 8)]));
    #pragma unroll
    for (int n = 0; n < 16; ++n) {
      int vkey = lane & 15;
      uint32_t vb0, vb1; ldm2t(vb0, vb1, sm(&Vs[cur][vkey][swz128(vkey, n * 8)]));
