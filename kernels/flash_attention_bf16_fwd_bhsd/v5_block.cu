...
// v5 block — v4's tensor-core math, but a BLOCK of WARPS=4 share one K/V tile.
// v4 was 1 warp/block (32 threads) → shared-limited, terrible occupancy. v5: 4 warps
// = 64 queries per block; K and V^T are loaded into shared ONCE per block and reused
// by all 4 query-warps. Halves shared-per-query (2× occupancy) and gives 4 warps to
// hide load/ldmatrix latency. Per-warp compute (QK^T, frag softmax, P-repack, PV) is
// identical to v4 — only Q/P are indexed by warp, K/V are block-shared.
// D=128, S_q%16==0, S_kv%16==0.
...
#define BNP 24
#define WARPS 4
#define QPB (WARPS * 16)        // queries per block = 64
...
fa_v5(const __nv_bfloat16* __restrict__ Q, const __nv_bfloat16* __restrict__ K,
  ...
  const int qblock = blockIdx.x * QPB;          // first query of this block
  const int tid = threadIdx.x, warp = tid >> 5, lane = tid & 31;
  const int group = lane >> 2, tidg = lane & 3;
  const int qbase = qblock + warp * 16;          // this warp's first query
  ...
  for (int kt = 0; kt <= kmax; kt += BN) {
    __syncthreads();
    for (int i = tid; i < 16 * DM; i += WARPS * 32) { int r = i / DM, c = i % DM; Ks[r][c] = K[kb + (long)(kt + r) * kss + c * ksd]; }
    for (int i = tid; i < DM * BN; i += WARPS * 32) { int d = i / BN, key = i % BN; Vt[d][key] = V[vb + (long)(kt + key) * vss + d * vsd]; }
    __syncthreads();
    ...
    for (int kk = 0; kk < 8; ++kk) {
      ...
      uint32_t b0, b1;
      ldm2(b0, b1, sm(&Ks[(lane & 7)][kcol]));         mma(s0, s1, s2, s3, a0, a1, a2, a3, b0, b1, s0, s1, s2, s3);
      ldm2(b0, b1, sm(&Ks[8 + (lane & 7)][kcol]));     mma(t0, t1, t2, t3, a0, a1, a2, a3, b0, b1, t0, t1, t2, t3);
    }
    float sc[8] = {s0, s1, t0, t1, s2, s3, t2, t3};
    int kcol0 = 2 * tidg;
    ...
    #pragma unroll
    for (int n = 0; n < 16; ++n) {
      uint32_t vb0, vb1; ldm2(vb0, vb1, sm(&Vt[n * 8 + (lane & 7)][((lane >> 3) & 1) * 8]));
